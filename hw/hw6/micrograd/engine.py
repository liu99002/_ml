...
class Value:
    ...
    def sigmoid (self):
        out = Value(1 / (1 + np.exp(-1 * self.data)), (self,), 'sigmoid')

        def _backward():
            self.grad += (out.data * (1 - out.data)) * out.grad # gx = 1 if f>0 else 0
        out._backward = _backward

    def exp (self):
        out = Value((np.exp(self.data)), (self,), 'sigmoid')

        def _backward():
            self.grad += (out.data) * out.grad # gx = 1 if f>0 else 0
        out._backward = _backward
